...
Reason: Fixed the crc size for checksum 3 and added the option to write:
	...
	Set a VideoFlags String
	Set a TimeZoneStdName String
	Set a TimeZoneDltName String
	Set a TimeZoneStdDate String
	Set a TimeZoneDltDate String
	Set a TimeZoneDltBias String
	Set a NullChecksum3 String
	DWriteToBINFile // decrypted eeprom.bin
--------------------------------------------------------------------------------------------------------
Date: 11/27/2004
By: Yoshihiro
...
/* Write the Current decrypted EEPROM image to a .BIN file.. */
BOOL XKEEPROM::DWriteToBINFile(LPCSTR FileName)
{
	BOOL retVal = FALSE;
	DWORD dwBytesWrote = 0;
...
/* Set Language in the form of BYTES in  a  Hex String representation */
void XKEEPROM::SetLanguageString(LPCSTR LanguageID)
{
	DWORD len = LANGUAGE_SIZE * 2;
	BYTE tmpData[(LANGUAGE_SIZE * 2) + 1];
	ZeroMemory(tmpData, len + 1);
	memcpy(tmpData, LanguageID, min(strlen(LanguageID), len));
...
/* Set VideoFlags in the form of BYTES in  a  Hex String representation */
void XKEEPROM::SetVideoFlagsString(LPCSTR VideoFlags)
{
	DWORD len = VIDEOFLAGS_SIZE * 2;
	BYTE tmpData[(VIDEOFLAGS_SIZE * 2) + 1];
	ZeroMemory(tmpData, len + 1);
	memcpy(tmpData, VideoFlags, min(strlen(VideoFlags), len));

	XKGeneral::HexStrToBytes(tmpData, &len, TRUE);

	memcpy(&m_EEPROMData.VideoFlags, tmpData, VIDEOFLAGS_SIZE);

	CalculateChecksum3();
}

/* Set Audio Settings in the form of BYTES in  a  Hex String representation */
void XKEEPROM::SetAudioFlagsString(LPCSTR AudioFlags)
{
	DWORD len = AUDIOFLAGS_SIZE * 2;
	BYTE tmpData[(AUDIOFLAGS_SIZE * 2) + 1];
	ZeroMemory(tmpData, len + 1);
	memcpy(tmpData, AudioFlags, min(strlen(AudioFlags), len));

	XKGeneral::HexStrToBytes(tmpData, &len, TRUE);

	memcpy(&m_EEPROMData.AudioFlags, tmpData, AUDIOFLAGS_SIZE);

	CalculateChecksum3();
}

/* Set Timezone Standard Date in the form of BYTES in  a  Hex String representation */
void XKEEPROM::SetTimeZoneStdDateString(LPCSTR TimeZoneStdDate)
{
	DWORD len = TIMEZONE_SIZE * 2;
	BYTE tmpData[(TIMEZONE_SIZE * 2) + 1];
	ZeroMemory(tmpData, len + 1);
	memcpy(tmpData, TimeZoneStdDate, min(strlen(TimeZoneStdDate), len));

	XKGeneral::HexStrToBytes(tmpData, &len, TRUE);

	memcpy(&m_EEPROMData.TimeZoneStdDate, tmpData, TIMEZONE_SIZE);

	CalculateChecksum3();
}

/* Set Timezone Day Light Time Date in the form of BYTES in  a  Hex String representation */
void XKEEPROM::SetTimeZoneDltDateString(LPCSTR TimeZoneDltDate)
{
	DWORD len = TIMEZONE_SIZE * 2;
	BYTE tmpData[(TIMEZONE_SIZE * 2) + 1];
	ZeroMemory(tmpData, len + 1);
	memcpy(tmpData, TimeZoneDltDate, min(strlen(TimeZoneDltDate), len));

	XKGeneral::HexStrToBytes(tmpData, &len, TRUE);

	memcpy(&m_EEPROMData.TimeZoneDltDate, tmpData, TIMEZONE_SIZE);

	CalculateChecksum3();
}

/* Set Timezone Day Light Time Bias in the form of BYTES in  a  Hex String representation */
void XKEEPROM::SetTimeZoneDltBiasString(LPCSTR TimeZoneDltBias)
{
	DWORD len = TIMEZONE_SIZE * 2;
	BYTE tmpData[(TIMEZONE_SIZE * 2) + 1];
	ZeroMemory(tmpData, len + 1);
	memcpy(tmpData, TimeZoneDltBias, min(strlen(TimeZoneDltBias), len));

	XKGeneral::HexStrToBytes(tmpData, &len, TRUE);

	memcpy(&m_EEPROMData.TimeZoneDltBias, tmpData, TIMEZONE_SIZE);

	CalculateChecksum3();
}

/* Set Timezone Standard in the form of String */
void XKEEPROM::SetTimeZoneStdNameString(LPCSTR TimeZoneStdName)
{
	DWORD len = TIMEZONE_SIZE;
	strncpy((LPSTR)&m_EEPROMData.TimeZoneStdName, TimeZoneStdName, len);

	CalculateChecksum3();
}

/* Set Timezone Day Light Time in the form of String */
void XKEEPROM::SetTimeZoneDltNameString(LPCSTR TimeZoneDltName)
{
	DWORD len = TIMEZONE_SIZE;
	strncpy((LPSTR)&m_EEPROMData.TimeZoneDltName, TimeZoneDltName, len);

	CalculateChecksum3();
}

/* Null the EEPROM data after checksum3 for factory reset of eeprom, in the form of BYTES in  a  Hex String representation */
void XKEEPROM::SetNullChecksum3String(LPCSTR Checksum3)
{
	DWORD len = NullChecksum3_SIZE * 2;
	BYTE tmpData[(NullChecksum3_SIZE * 2) + 1];
	ZeroMemory(tmpData, len + 1);
	memcpy(tmpData, Checksum3, min(strlen(Checksum3), len));

	XKGeneral::HexStrToBytes(tmpData, &len, TRUE);

	memcpy(&m_EEPROMData.Checksum3, tmpData, NullChecksum3_SIZE);
}
