...
#define NKPSShadowCPartition								"14"
...
HRESULT ConfigMagicApp::Initialize()
{
	...
	if (VirtualEEPROM.good())
	{
		...
		V_EEPROM.seekp(52);
		V_EEPROM.write("\x56\x2D\x45\x45\x50\x52\x4F\x4D\x20\x45\x4E\x42", 12); // Writes "V-EEPROM ENB" as the serial number, need to add recalculating the CRC as it breaks the MAC address.
		V_EEPROM.close();
	}
		else
	{
 ...
 */
		...
		/*
			First run code, this will patch UnleashX, create a clean ShadowC.img.
		*/
		...
		/*
			Update run code, this will patch UnleashX.
		*/
...
void ConfigMagicApp::XBEFiles()
{
	...
	UnleashX_XBE4.write(ERescueDashHabibi, 260);
	// Change XBE Title ( E Recovery Dash )
	UnleashX_XBE4.seekp(400);
	UnleashX_XBE4.write(ERescueDashLabel, ERescueDashLabelSize);
	// XBE Hash
	UnleashX_XBE4.seekp(1532);
	UnleashX_XBE4.write(ERescueDashHash, 20);
	// Remove recent.dat creation
	UnleashX_XBE4.seekp(2353004);
	UnleashX_XBE4.write(RemoveRecentdat, RemoveRecentdatSize);
	UnleashX_XBE4.close();
	...
	UnleashX_XBE6.write(DashSettingsHabibi, 260);
	// Change XBE Title ( Backup, Restore or Move )
	UnleashX_XBE6.seekp(400);
	UnleashX_XBE6.write(DashSettingsLabel, DashSettingsLabelSize);
	// XBE Hash
	UnleashX_XBE6.seekp(1532);
	UnleashX_XBE6.write(DashSettingsHash, 20);
	// Remove recent.dat creation
