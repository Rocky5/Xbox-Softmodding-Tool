		...
		if(m_XBOX_EEPROM_Current)
		{
			...
			if (m_pXKEEPROM->Decrypt())
			{
				...
				if ((SecStatus & IDE_SECURITY_SUPPORTED) && (SecStatus & IDE_SECURITY_ENABLED))
				{
					...
					{
						m_ActiveForm = m_pFrmStatus;
						((LPXKControl_TextBox) m_pFrmStatus->GetControl("txtStatusMsg"))->SetText("Remenber MASTER PASS: TEAMASSEMBLY");
			...
			if (m_pXKEEPROM->Decrypt())
			{
				...
				if ((SecStatus & IDE_SECURITY_SUPPORTED) && !(SecStatus & IDE_SECURITY_ENABLED))
				{
					...
					*pMastr = 0x0001; //Set Master Pwd..
					...
					{
						m_ActiveForm = m_pFrmStatus;
						((LPXKControl_TextBox) m_pFrmStatus->GetControl("txtStatusMsg"))->SetText("Remember Master PASS : TEAMASSEMBLY ");
...
void ConfigMagicApp::ExitScreen()
{
	...
	if (!WaitForInput)
	{
		Render();
		m_ActiveForm = m_pFrmStatus;
		m_ActiveForm->LoadTransparentImage("D:\\Media\\done.jpg", D3DCOLOR_ARGB(0xFF, 0xFF, 0x00, 0xFF));
		WaitForInput = TRUE;
	}
	else
	{
...
//	XConfSections = STARTUP_SCREEN;
	...
	retVal = m_XBFont16.Create(m_pd3dDevice, "D:\\Media\\Gunplay_20.xpr");
	retVal = m_XBFont20.Create(m_pd3dDevice, "D:\\Media\\Gunplay_20.xpr");
	retVal = m_XBFont24.Create(m_pd3dDevice, "D:\\Media\\Gunplay_20.xpr");
   ...
   DirectSoundCreate( NULL, &m_pDSound, NULL );
	...
	m_pMainMenu->LoadTransparentImage("D:\\Media\\Menu.bmp", D3DCOLOR_ARGB(0xFF, 0xFF, 0x00, 0xFF));
	m_pMainMenu->SetTitleOffset(0, 5);
	m_pMainMenu->SetTitleTextFont(&m_XBFont20);
	m_pMainMenu->SetItemsOffset(5, 80); //XOffset is always from Right to exclude border..
	m_pMainMenu->SetItemTextFont(&m_XBFont16);
	
	m_kbdEditBox = new XKControl_Keyboard("kbdEditBox", m_bgSprite, &m_XBFont24, XOffset + ((ScreenWidth/2) - (m_KeyboardWidth/2)), YOffset + ((ScreenHeight/2) - (m_KeyboardHeight/2)), m_KeyboardWidth, m_KeyboardHeight);
	m_kbdEditBox->LoadTransparentImage("D:\\Media\\keyboard1.bmp", D3DCOLOR_ARGB(0xFF, 0xFF, 0x00, 0xFF));
	m_kbdEditBox->SetStandardKeyboardLayout(XKControl_Keyboard::TYPE_FULL);
	m_kbdEditBox->SetVisible(FALSE);
	m_kbdEditBox->SetEditTextFont(&m_XBFont20);
...
void ConfigMagicApp::LoadMainMenuItems()
{
	...
	m_pMainMenu->AddMenuItem("I_BLANK", " ");
...
void ConfigMagicApp::InitStatusScreen()
{
	...
	m_pFrmStatus->LoadImage("D:\\Media\\Status.jpg");
	
	txtStatusMsg = new XKControl_TextBox("txtStatusMsg", m_bgSprite);
	txtStatusMsg->SetBitmapFont(&m_XBFont24);
	txtStatusMsg->SetTextColor(m_clrMessageTextColor);
	txtStatusMsg->SetTextCentered(TRUE);
	txtStatusMsg->SetTextShadow(TRUE);

	m_pFrmStatus->AddControl(txtStatusMsg,  0, 185,  ScreenWidth, 30);
...
void ConfigMagicApp::InitConfigScreen()
{
	...
	txtMAC->SetBitmapFont(&m_XBFont16);
	...
	txtVideoFormat->SetBitmapFont(&m_XBFont16);
	...
	txtOnlineKey->SetBitmapFont(&m_XBFont16);
	...
	txtDVDZone->SetBitmapFont(&m_XBFont16);
	...
	txtDVDModel->SetBitmapFont(&m_XBFont16);
	...
	txtHDDKey->SetBitmapFont(&m_XBFont16);
	...
	txtHDDModel->SetBitmapFont(&m_XBFont16);
	...
	txtHDDSerial->SetBitmapFont(&m_XBFont16);
	...
	txtHDDStatus->SetBitmapFont(&m_XBFont16);
	txtHDDStatus->SetTextColor(m_clrValuesTextColor);
	txtHDDStatus->SetTextCentered(TRUE);
	txtHDDStatus->SetTextShadow(TRUE);


	txtHDDPwd = new XKControl_TextBox("txtHDDPwd", m_bgSprite);
	txtHDDPwd->SetBitmapFont(&m_XBFont16);
...
void ConfigMagicApp::LoadSettingsFromINI()
{
	...
	tmpLen = FILENAME_MAX;
	ZeroMemory(tmpINIItem, tmpLen);
	if (XKGeneral::ReadINIFileItem(MainConfigFileName, "OUTPUT", "EEPROM_Path", (LPSTR)tmpINIItem, &tmpLen))
	{
		XKGeneral::StripQuotes((LPSTR)tmpINIItem, &tmpLen);	
		strncpy((LPSTR)m_EEPROM_Backup_Path, (LPCSTR)tmpINIItem, tmpLen);
	}
	else
		strcpy((LPSTR)m_EEPROM_Backup_Path, "E:");
	...
	m_clrMessageTextColor		= 0xFFB9C6CC;
	tmpLen = FILENAME_MAX;
	ZeroMemory(tmpINIItem, tmpLen);
