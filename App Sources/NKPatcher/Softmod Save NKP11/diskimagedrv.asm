...
installdiskimagedrv:	
;---------------------------------------
;	lets see if were here from a tray open boot
;---------------------------------------
	pushad				; save machine state
        cmp     byte [tray_data1], 10h	; 10h = open at boot, anything else was closed 
	jne     short tst_sc.		; test shadowC off file (was NOT tray open boot)
	mov	ebx,[options+4]		; get tray options [TRAY_VAL]
	test	ebx,16			; mask for OPEN_shadowC_OFF
	jne	short sc_off. 		; done, no shadow C
;---------------------------------------
;	lets see if the override file is there
;---------------------------------------
tst_sc.	lea	eax,[sc_fp]		; shadow C off file name
	call	open_file		; go try to open eeprom_off file
	cmp     [esi+4],dword 01h	; status was put here by NtOpenfile (8 bytes total)
	jne	short ins_dd.		; file not there, install shadow C
;---------------------------------------
	push	dword [ebx]
	call	dword [NtClose]
sc_off.	popad				; unwind stack
	ret				; were done, no shadow C
;---------------------------------------
ins_dd.	popad				; restore machine state
	mov	edi,[expd_space]
	test	edi,edi
	jz	near .exit
	mov	esi,[memdiff]
